	...
	*/
	...
	function admin_head()
	{
		...
		if( isset($_POST['acf_nonce']) && wp_verify_nonce($_POST['acf_nonce'], 'input') )
		{
			...
			do_action('acf/save_post', apply_filters('acf/get_post_id', 'options' ));
			// =duepi.biz
		...
		if( $acfs )
		{
			foreach( $acfs as $acf )
			{
				...
				add_meta_box(
					...
					array( 'field_group' => $acf, 'show' => $show, 'post_id' => apply_filters('acf/get_post_id', 'options' ) )
					// =duepi.biz
				);
...
// +duepi.biz
add_filter( 'acf/get_post_id', 'custom_get_post_id', 99, 1);

function custom_get_post_id( $post_id ) {

	if ( defined('ICL_LANGUAGE_CODE') ) {
		if( $post_id =='options' ) {
			$post_id =ICL_LANGUAGE_CODE .'_'. 'options';
		}
	}
